    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-10">
        <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 mb-14">
            ...
            <div class="footer-reveal" style="transition-delay: 300ms;">
                ...
                <form id="newsletter-form" action="{{ route('landing.newsletter.subscribe') }}" method="POST" class="space-y-3">
                    @csrf
                    <div class="relative group">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <i class="fa-regular fa-envelope text-slate-500 group-focus-within:text-secondary-500 transition-colors"></i>
                        </div>
                        <input type="email" name="email" id="newsletter-email" required placeholder="Your email address" class="newsletter-input w-full pl-10 pr-4 py-3 bg-slate-800 border border-slate-700 text-white placeholder-slate-500 outline-none transition-all duration-300 focus:border-secondary-500 focus:shadow-[0_0_0_3px_rgba(147,51,234,0.15)]">
                    </div>
                    <button type="submit" id="newsletter-submit" class="w-full bg-gradient-to-r from-secondary-600 to-secondary-700 text-white py-3 rounded-sm font-bold text-sm uppercase tracking-wide hover:from-secondary-700 hover:to-secondary-800 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 flex items-center justify-center gap-2">
        ...
        <div class="border-t border-slate-800 pt-8 flex flex-col md:flex-row items-center justify-between gap-4 text-sm">
            ...
            <div class="flex items-center gap-6">
                <a href="{{ route('sitemap.html') }}" class="footer-bottom-link text-slate-500 hover:text-white transition-colors relative">Sitemap</a>
