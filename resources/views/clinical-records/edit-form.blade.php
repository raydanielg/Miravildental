...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    <div>
        ...
        <select name="patient_id" class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 outline-none focus:border-emerald-500" required>
            ...
            <option value="{{ $p->id }}" @selected(old('patient_id', $clinicalRecord->patient_id) == $p->id)>{{ $p->name }}</option>
            @endforeach
        </select>
    </div>
    <div>
        ...
        <select name="service_id" class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 outline-none focus:border-emerald-500">
            ...
            <option value="{{ $s->id }}" @selected(old('service_id', $clinicalRecord->service_id) == $s->id)>{{ $s->name }}</option>
            @endforeach
        </select>
    </div>
    <div>
        <label class="block text-xs font-medium text-gray-700 mb-1">Record Date</label>
        <input type="date" name="record_date" value="{{ old('record_date', $clinicalRecord->record_date?->format('Y-m-d')) }}" class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 outline-none focus:border-emerald-500" required>
    </div>
    <div>
        <label class="block text-xs font-medium text-gray-700 mb-1">Cost (TZS)</label>
        <input type="number" name="cost" value="{{ old('cost', $clinicalRecord->cost) }}" class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 outline-none focus:border-emerald-500">
    </div>
    <div>
        <label class="block text-xs font-medium text-gray-700 mb-1">Linked Appointment</label>
...
<div class="mt-6 flex items-center gap-2">
    <button type="submit" class="px-4 py-2 text-xs font-medium bg-emerald-600 text-white rounded-lg hover:bg-emerald-700">Update Record</button>
    <button type="button" onclick="closeEditClinicalRecordModal()" class="px-4 py-2 text-xs font-medium border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Cancel</button>
</div>
